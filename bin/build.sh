...
electron-packager ./ ${PACKAGE_NAME} --platform=win32 --arch=ia32 --version="${ELECTRON_VERSION}" --app-version="${PACKAGE_VERSION}" --icon=treasure_logo.ico &&
zip -r "${ZIP_WIN}" "${PACKAGE_NAME}-win32-ia32"
cp ${ZIP_WIN} ./../zips/
rm -rf "${PACKAGE_NAME}-win32-ia32*" &&
electron-packager ./ ${PACKAGE_NAME} --platform=darwin --arch=x64 --version="${ELECTRON_VERSION}" --app-version="${PACKAGE_VERSION}" --icon=treasure_logo.icns &&
